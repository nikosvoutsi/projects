...
<div class="wrapper fadeInDown">
    ...
    <div id="form">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $errors=[];
            if(!$work=filter_input(INPUT_POST,'work', FILTER_SANITIZE_STRING)){
                $errors[]='Please insert work';
            }
            if(!$company=filter_input(INPUT_POST,'company', FILTER_SANITIZE_STRING)){
                $errors[]='Please insert company';
            }
            ...
            if($date2<$date1){
               print"<p style='color:red'>Η τελική ημερομηνία πρέπει να είναι μεταγενέστερη της αρχικής!!!</p>\n";
                ...
                //print"$id";
                $query2='insert into work_experience (idpeople, work, company, start, end)
                values (?,?,?,?,?)';
                $stmt2=mysqli_prepare($dbc, $query2);
                mysqli_stmt_bind_param($stmt2, 'issss', $id, $work, $company,  $date1, $date2);
        ...
        <form action='work2.php' method='post' enctype='multipart/form-data'>
