  ...
  <div id="formContent">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if(!empty($errors)){
            ...
            $query = "UPDATE users SET uname=?, password=? WHERE am=?";
            $stmt = my_mysqli_prepare($dbc, $query);
            my_mysqli_stmt_bind_param($stmt, 'ssi', $uname, $password1, $id);
            my_mysqli_stmt_execute($stmt);
            if (mysqli_stmt_affected_rows($stmt)!=1){
                print"<p style='color:red;'>H εγγραφή δεν ενημερώθηκε!</p>\n";
            }else{
